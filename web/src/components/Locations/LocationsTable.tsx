...
import { GooglePlacesSyncPanel } from "./GooglePlacesSyncPanel";
...
// entrepôts, hubs 3PL), capacité (emplacements palettes), création/édition,
// désactivation, synchronisation automatique Google Maps, et import/export
...
export function LocationsTable() {
  ...
  function startEdit(location: Location) {
    ...
    setForm({
      ...
      capacityPallets: location.capacityPallets != null ? String(location.capacityPallets) : "",
  ...
  async function handleSubmit(event: FormEvent) {
    ...
    const payload = {
      ...
      latitude,
      longitude,
      capacityPallets: form.capacityPallets ? Number(form.capacityPallets) : undefined,
  ...
  async function handleGeocode(location: Location) {
    ...
    try {
      ...
      setGeocodeError(`${location.name} : ${err instanceof ApiError ? err.message : "Échec du géocodage."}`);
    } finally {
      setGeocodingId(null);
    }
  }
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between flex-wrap gap-3">
        ...
        <div className="flex flex-wrap gap-2">
          <GooglePlacesSyncPanel onImported={reload} />
          <button
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <form onSubmit={handleSubmit} className="lg:col-span-1 bg-white rounded-xl border border-slate-200 p-5 space-y-3 h-fit">
          ...
          <Field label="Capacité (emplacements palettes)">
            <input
              className="input"
            ...
            <table className="w-full text-sm">
              <thead className="bg-slate-50 text-slate-500 text-xs uppercase">
                <tr>
                  ...
                  <th className="text-right px-4 py-3">Capacité</th>
                ...
                {filtered.map((location) => (
                  <tr key={location.id} className="border-t border-slate-100">
                    ...
                    <td className="px-4 py-3 text-right text-slate-500">
                      {location.capacityPallets != null ? `${location.capacityPallets.toLocaleString()} pal.` : "—"}
                    </td>
                    <td className="px-4 py-3">
