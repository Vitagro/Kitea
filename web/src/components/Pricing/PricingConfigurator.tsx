...
export function PricingConfigurator() {
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
      <form onSubmit={handleSubmit} className="lg:col-span-1 bg-white rounded-xl border border-slate-200 p-5 space-y-4 h-fit">
        ...
        <div className="space-y-1">
          ...
          <input className="input" value={form.label} onChange={(e) => setForm({ ...form, label: e.target.value })} placeholder="ex: Tarif interne au km - Zone Nord" />
        </div>

        <div className="space-y-1">
          <label className="text-xs font-medium text-slate-500">Type de règle</label>
          <select className="input" value={form.ruleType} onChange={(e) => setForm({ ...form, ruleType: e.target.value as PricingRuleType })}>
        ...
        <div className="space-y-1">
          ...
          <select className="input" value={form.source} onChange={(e) => setForm({ ...form, source: e.target.value as PricingSource })}>
        ...
        {form.ruleType === "TRANSPORT_FLAT_ZONE" && (
          <div className="space-y-1">
            ...
            <input className="input" value={form.zoneName} onChange={(e) => setForm({ ...form, zoneName: e.target.value })} placeholder="ex: Zone Nord" />
          </div>
        )}
            ...
            {rules.map((rule) => (
              <tr key={rule.id} className="border-t border-slate-100">
                ...
                <td className="px-4 py-3">
                  <span className={`text-xs rounded-full px-2 py-0.5 ${rule.source === "INTERNAL" ? "bg-blue-50 text-blue-700" : "bg-amber-50 text-amber-700"}`}>
                ...
                <td className="px-4 py-3 text-right">
                  <button onClick={() => handleDeactivate(rule.id)} className="text-slate-400 hover:text-red-600" title="Désactiver">
                    <Trash2 size={16} />
                  </button>
                </td>
              </tr>
