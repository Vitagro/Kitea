    ...
    <div className="bg-white rounded-xl border border-slate-200 overflow-hidden">
      <div className="p-5 border-b border-slate-100">
        ...
        <p className="text-sm text-slate-500">
          Comparaison du coût théorique calculé vs le montant facturé par le prestataire. Les pré-factures
          se génèrent depuis l'écran "Expéditions".
        </p>
      </div>
                ...
                <tr className="border-t border-slate-100">
                  ...
                  <td className="px-4 py-3 text-right space-x-2 whitespace-nowrap">
                    {pi.matchingStatus === "PENDING_INVOICE" && (
                      <button
                        onClick={() => startAttach(pi.id)}
                        className="inline-flex items-center gap-1 text-xs rounded-md bg-kitea-blue text-white px-2 py-1"
                      >
                    ...
                    <td colSpan={6} className="px-4 py-4">
                      <form onSubmit={(e) => handleAttachSubmit(e, pi.id)} className="flex flex-wrap items-end gap-3">
                        <div className="space-y-1">
                          ...
                          <select
                            className="input"
                            value={invoiceForm.carrierId}
                            onChange={(e) => setInvoiceForm({ ...invoiceForm, carrierId: e.target.value })}
                          >
